...
const generateFileTree = (dir, prefix = '') => {
  ...
  items.forEach((item, index) => {
    if (item.name === 'node_modules' || item.name === '.git' || item.name === '.next') return; // Пропускаем node_modules и .git
...
const collectFiles = (dir, collectedFiles = []) => {
  ...
  items.forEach(item => {
    if (item.name === 'codewr.js' || item.name === 'node_modules' || item.name === '.git' || item.name === '.next') return; // Пропускаем node_modules и .git
